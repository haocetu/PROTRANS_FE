...
function OrderNoPick() {
  ...
  const handleUpdateOrderStatus = async (orderId: string) => {
    try {
      // Gọi API PUT
      const response = await api.put(`Order/UpdateToPickedUp/${orderId}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (id, data) => (
        <Popconfirm
          title="Bạn có xác nhận tiếp nhận đầy đủ bản gốc tài liệu của đơn hàng này?"
          okText="Xác nhận"
          cancelText="Hủy"
  ...
  async function fetchOrder() {
    ...
    const response = await api.get(
      `Order/GetOrdersToReceive?staffId=${account.Id}`
    );
    console.log(response.data.data);
